window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SpeakControl(props) {
			// dsh 0.1.2 moved message payloads out of the session snapshot into
			// the chat projection (useChat): nodes are keyed by c.order and the
			// assistant payload ({kind:"assistant", messageId, blocks, time})
			// sits nested inside a chat node. Field names are unchanged, so a
			// bounded recursive scan finds it and stays robust to re-nesting.
			function findAssistantPayload(c, messageId) {
				function scan(v, depth) {
					if (!v || typeof v !== "object" || depth > 14) return null;
					if (v.messageId === messageId && Array.isArray(v.blocks)) return v;
					for (var k in v) {
						var r = scan(v[k], depth + 1);
						if (r) return r;
					}
					return null;
				}
				// Chat snapshot members are class instances whose payloads only
				// surface through toJSON(), so live property traversal misses
				// them — scan the serialized tree instead.
				var plain;
				try { plain = JSON.parse(JSON.stringify(c)); } catch (e) { return null; }
				return scan(plain, 0);
			}
			var text = props.useChat(function (c) {
				var m = findAssistantPayload(c, props.messageId);
				return m ? blocksToText(m.blocks) : "";
			});
			var time = props.useChat(function (c) {
				var m = findAssistantPayload(c, props.messageId);
				return m ? m.time : 0;
			});
		...
		function apply(ctx) {
			...
			// dsh 0.1.2 slot contract: ONE inject per slot (a second inject on
			// the same name replaces the first), callback returns the disposer;
			// entries ordered by `order`. `priority` is a separate axis: two entries
			// with the same id AND the same priority (default 0) are a hard error, and
			// among same-id entries the LOWEST priority renders. So shadowing the
			// shipped "feedback" cell needs priority < 0 — `order` alone collides
			// (verified on dsh 0.1.1-rc.2 and 0.1.2-rc.1).
			ctx.slots.inject("conversation.chat.assistant-actions", function () {
				var disposeSpeak = ctx.slots.register({ name: "conversation.chat.assistant-actions", id: "voice-speak", order: 20 }, SpeakControl);
				var disposeFeedback = ctx.slots.register({ name: "conversation.chat.assistant-actions", id: "feedback", order: -10, priority: -1 }, NullCell);
				return function () { disposeSpeak(); disposeFeedback(); };
			});
		}
		exports.apply = apply;
